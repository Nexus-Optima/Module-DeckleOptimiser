...
def customer_table(final_dict, df):
    ...
    customer_df[['ITEM', 'SO', 'CUSTOMER', 'CONSIGN', 'WIDTH', 'ID','TARGET ROLL']] = df[['ITEM NO', 'SO', 'CUSTOMER', 'CONSIGN', 'WIDTH', 'ID','NO.OF ROLL']]
    ...
    for i in range(len(customer_df)):
        if customer_df['WIDTH'].iloc[i] in final_dict.keys():
            if final_dict.get(customer_df['WIDTH'].iloc[i]) >= customer_df['TARGET ROLL'].iloc[i]:
                ...
                final_dict[customer_df['WIDTH'].iloc[i]] = final_dict[customer_df['WIDTH'].iloc[i]] - customer_df['TARGET ROLL'].iloc[i]
            else:
    ...
    customer_df['PROD QTY'] = customer_df['ACTUAL ROLL'] * customer_df['WIDTH'] * df['MIC']*df['SAP LENGTH']*df['DENSITY']/1000000
