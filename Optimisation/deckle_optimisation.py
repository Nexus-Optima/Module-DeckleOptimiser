...
def optimise_residual_deckle(values, option_and_must_dict, residual_dict, position, max_width, min_arms,
    ...
    try:
        while True:
            ...
            obj_value_waste = LpVariable("obj_value_waste", lowBound=0, cat="Continuous")
            prob += alpha * (obj_value_waste - 50) / 75 + (1 - alpha) * (obj_value - 0.5) / 0.35
            prob += obj_value_waste == max_width - lpSum(choices[p][v] * v for v in values for p in position)
            ...
            prob += max_width - lpSum(choices[p][v] * v for v in values for p in position) <= 5000
            ...
            if LpStatus[prob.status] == 'Optimal':
                ...
                final_list_deckle.append(optimised_deckle)
                final_list_deckle, option_and_must_dict, residual_dict = get_repetitive_deckle_optional(
                    final_list_deckle,
                    optimised_deckle,
...
def get_optimised_deckle(values, number_dict, position, max_width, min_arms, alpha):
    final_list_deckle = []
    while True:
        optimised_deckle = []
        prob = LpProblem("Deckle_Problem", LpMinimize)
        ...
        for v in values:
            if number_dict.get(v) > 0:
                ...
                prob += lpSum(choices[p][v] for p in position) / number_dict.get(v) <= 1 / 3
        for p in position:
            if p <= min_arms:
                prob += lpSum([choices[p][v] for v in values]) == 1
            else:
        ...
        prob += max_width - lpSum(choices[p][v] * v for v in values for p in position) >= 0
        # total width threshold as 200
        prob += max_width - lpSum(choices[p][v] * v for v in values for p in position) <= 150
        prob.solve(PULP_CBC_CMD(timeLimit=5))
        if LpStatus[prob.status] == 'Optimal':
            for p in position:
                for v in values:
