    ...
    if LpStatus[prob.status] == 'Optimal':
        ...
        for v in prob.variables():
            if v.varValue > 0:
                print(v.name, "=", v.varValue)
    status = prob.status
    return optimised_deckle, option_and_must_dict, residual_dict, status
...
def optimise_deckle(data, minimum_trim):
    ...
    print("Now starting with optional values")
    flag = 1
    while flag == 1:
        op_deckle, option_and_must_dict, residual_dict, flag = optimise_residual_deckle(option_and_must_values,option_and_must_dict,residual_dict,position,possible_width, prms.min_arms)
        op_deckle = sorted(op_deckle)
        final_list_deckles.append(op_deckle)
